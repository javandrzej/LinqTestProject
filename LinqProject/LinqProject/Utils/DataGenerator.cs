...
namespace LinqProject.Utils
{
    public class DataGenerator : IGenarator
    {
        ...
        {
            return Enumerable.Range(1, count).Select(p => new Person(Guid.NewGuid(), GenerateRandomString(10, Letters.small), GenerateRandomString(15, Letters.small), 20 + p, "myemail" + p + p, DateTime.Now.Add(new TimeSpan(p, 0, 0, 0))));
        }
